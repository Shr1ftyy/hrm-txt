...
class HRMText1(nn.Module):
    ...
    def forward(self, input_ids, labels=None, attention_mask=None):
        ...
        z_L = self.token_embeddings(input_ids) + self.pos_embeddings(
            self.pos_ids[:, :seq_len]
        )
        ...
        for step in range(self.max_steps):
            ...
            remainders = (
                remainders * (1 - p_halt)
                if not is_last_step
                else torch.zeros_like(remainders)
            )
            ...
            z_H, z_L = self.inner_model(
                z_H, z_L, attn_mask=causal_mask, key_padding_mask=key_padding_mask
            )
